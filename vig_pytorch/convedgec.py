...
class edgec(nn.Module):
    ...
    def __init__(self, in_dim=3, out_dim=768):
        ...
        self.conv1 = nn.Conv2d(in_dim, in_dim, kernel_size=1, bias=False)
        self.conv2 = nn.Conv2d(in_dim, out_dim, kernel_size=3, stride=1, padding=1, dilation=1, groups=4,bias=True)
        self.conv3 = nn.Conv2d(out_dim, out_dim, kernel_size=1, bias=False)
        self.act = nn.LeakyReLU(negative_slope=0.2)
        # self.conv1 = nn.Sequential(nn.Conv2d(in_dim, in_dim, kernel_size=1, bias=False),
        #                            nn.Conv2d(in_dim, out_dim, kernel_size=3, bias=False,groups=4),
        #                            nn.Conv2d(out_dim, out_dim, kernel_size=1, bias=False),
        #                            self.bn1,
        #                            nn.LeakyReLU(negative_slope=0.2))

    def forward(self, x):
        # x = self.act(self.bn1(self.conv3(self.conv2(self.conv1(x)))))
...
class DeepGCN(torch.nn.Module):
    def __init__(self, opt):
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, channels[0], 224 // 8, 224 // 8))
        HW = 224 // 8 * 224 // 8
    ...
    def forward(self, inputs):
        ...
        x = F.adaptive_avg_pool2d(x+idn, 1)
